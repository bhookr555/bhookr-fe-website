...
function renderCell(
  ...
  if (key === "timestamp" || key === "paymentTimestamp") {
  ...
  if (key === "subtotal" || key === "deliveryBase" || key === "itemGST" || key === "deliveryGST") {
  ...
  if (key === "paymentStatus") {
    ...
    const styles: Record<string, string> = {
      ...
      refunded: "bg-purple-100 text-purple-800 dark:bg-purple-950/40 dark:text-purple-300",
    };
    return (
      <span
  ...
  if (key === "items") {
    return (
      <span className="block max-w-[320px] truncate" title={String(value ?? "")}>
        {String(value ?? "") || "—"}
      </span>
    );
  }
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="mt-0.5 text-2xl font-bold text-gray-900 dark:text-white">
            {loading ? "Loading…" : `${filtered.length} of ${rows.length} orders · ${formatINR(totalRevenue)}`}
          </h1>
          {!loading && (
            <p className="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
              {totalItems} items sold in current view · last updated{" "}
      ...
      <div className="flex flex-wrap items-center gap-2">
        <input
          ...
          placeholder="Search customer, email, phone, order ID, item…"
          className="flex-1 min-w-[220px] rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-gray-900 placeholder:text-gray-400 focus:border-[#E31E24] focus:outline-none focus:ring-1 focus:ring-[#E31E24] dark:border-gray-800 dark:bg-gray-900 dark:text-white sm:max-w-md"
        />
        <select
          value={paymentFilter}
              ...
              <tr>
                ...
                {ORDER_COLUMNS.map((col) => (
                  <th
                    ...
                    className={`whitespace-nowrap border-b border-gray-200 px-3 py-2 text-[11px] font-semibold uppercase tracking-wider text-gray-500 dark:border-gray-800 dark:text-gray-400 ${col.align === "right" ? "text-right" : "text-left"}`}
                  >
                    {col.label}
                  </th>
                ))}
                ...
                filtered.map((row, idx) => (
                  <tr key={`${row.orderId}-${idx}`} className="odd:bg-white even:bg-gray-50 hover:bg-red-50/40 dark:odd:bg-gray-900 dark:even:bg-gray-950 dark:hover:bg-red-950/20">
                    ...
                    {ORDER_COLUMNS.map((col) => (
                      <td
                        key={col.key}
                        className={`whitespace-nowrap border-b border-gray-100 px-3 py-2 text-gray-700 dark:border-gray-800 dark:text-gray-200 ${col.align === "right" ? "text-right" : "text-left"}`}
                      >
                        {renderCell(row, col.key, col.align)}
                      </td>
                    ))}
                  </tr>
                ))
